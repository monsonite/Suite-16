...
Non-register OPS-
     ...
     09        RET    Return
     0A        ADI    Add 8-bit Immediate           Immediaate = IR7:0
     0B        SBI    Subtract 8-bit Immediate      Immediaate = IR7:0
     0C        OUT                                  putchar(AC)
     0D        IN                                   AC = getchar()
     0E        JP@                                  BRA (R0)
     0F        NOP                                  AC &= AC
...
unsigned int M[MEMSIZE] = {
   ...
   // 0x0000 ---------------------------SIMPL------------------------------------------ 
    ...
    //  R6 holds the execution address for the ALPHA command  
        ...
        0x0E00,     // JMP @R0      Jump to the command routine
        0x0008,     // BRA 0x0008 NEXT   

        // 0x001C -----------------------------GETHEX----------------------------

        0x1800,    // SET R8, 0x005B   Value for "G"
        0x0047,
        0x1900,    // SET R9, 0x0040   Value for "@"
        0x0040,
       ...
       // 0x0020--------------------------------------------------------------------------------------
        ...
        0x1200,    // SET R2, 0x005B   Value for "["
        0x005B,
        0x1300,    // SET R3, 0x0030   Value for " "
        0x0030,
        ...
        0x022F,     // BLT Not a digit
        
        0xB200,     // Subtract R2  0x0A
        0x032F,     // BGE Not a digit
        0x3A00      // ST R0, R10   store the digit in R10
        0x2400,     // LD  R0, R4   Get the command character back in R0

// 0x0030--------------------------------------------------------------------------------------
        
        0xB900,     // Subtract R9 Is it bigger than 0x40?
        0x025A,     // BLT Not a HEX
        0x2400,     // LD  R0, R4   Get the command character back in R0            
        0xB800,     // Subtract R8  0x5B
              
        0x0318,     // BGE Not a HEX


        0x0F00,     // NOP  If you arrive here you have a HEX in R4
        0x2400,     // LD  R0, R4   Get the command character back in R0   
        0x1B00      // SET R11  0x37   (decimal 55 or ascii '7')
        0x0037
        0xBB00      // SUB R11   Make it a number between 10 and 16
        
        0x3A00      // ST R0, R10  and put into R10
        0x0F00,     // NOP
...
// 0x00B0 ----------------------------------------------------------------
 ...
 //  It stops when it finds the null terminator
        ...
        0x013C,     // BNZ 0x3C loop around until you find 0 terminator
